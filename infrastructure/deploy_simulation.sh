#!/bin/bash
# Docker build and push script for c23-networkx-pipeline
# Gets values from Terraform outputs and builds from parent directory

set -e

# Get outputs from Terraform (in current directory)
ECR_REPO_URL=$(terraform output -raw ecr_repository_url_simulation)
ECR_REPO_NAME=$(terraform output -raw ecr_repository_name_simulation)
