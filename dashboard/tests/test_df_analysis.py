...
def test_remove_uninfluenced_stations_removes_user_station_rows(comparison_df_2):
    result = remove_uninfluenced_stations(comparison_df_2)

    assert len(result) == 1
    assert result.iloc[0]["route_id"] == 3


def test_get_passenger_station_counts(comparison_df_2):
    result = get_passenger_station_counts(comparison_df_2)
...
def test_get_affected_routes_combines_bidirectional_routes(comparison_df_2):
    result = get_affected_routes(comparison_df_2)
...
def test_get_demand_impact_ranges_returns_station_ranges(comparison_df_2):
    result = get_demand_impact_ranges(comparison_df_2)
...
def test_total_time_spent_diff(comparison_df_2):
    assert get_total_time_spent_diff(comparison_df_2) == -6.0
...
def test_greatest_time_spent_diff(comparison_df_2):
    assert get_greatest_time_spent_diff(comparison_df_2) == -5.0
...
def test_get_percentage_of_affected_routes(
    ...
    result = get_percentage_of_affected_routes(comparison_df_2, number_of_routes)

    assert result == expected_percentage


def test_create_top_affected_routes_chart_returns_altair_chart(comparison_df_2):
    result = create_top_affected_routes_chart(comparison_df_2)

    assert isinstance(result, alt.Chart)


def test_create_top_time_saving_routes_chart_returns_altair_chart(comparison_df_2):
    result = create_top_time_saving_routes_chart(comparison_df_2)

    assert isinstance(result, alt.Chart)


def test_create_station_demand_impact_chart_returns_altair_chart(comparison_df_2):
    result = create_station_demand_impact_chart(comparison_df_2)
